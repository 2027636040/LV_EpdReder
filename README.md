...
配置和城市有效且 PAN 接通后进行一次静默同步，之后按固定 30 分钟周期更新。
后台请求不弹窗，页面只更新实际变化的字段；缺失数据使用 `--`。
...
阅读页顶部显示返回图标和书名，正文为 y=174 至 y=1119，底部显示进度条及页码。
页面高度参与分页缓存校验，正文区域调整后按原文件位置重建分页并恢复进度。
阅读页只在内容或操作状态变化时更新正文与页脚，不显示分钟时钟或按键提示，不使用动画或渐变。
...
“保存设置”显示确认弹窗，关闭后恢复保存按钮焦点；通用设置及文本设置均通过
顶部返回图标保留当前选择并返回上一级。阅读字体、字号、行距、边距和编码参与真实排版，
